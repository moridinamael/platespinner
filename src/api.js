...
export const api = {
  ...
  login: (token) => request('POST', '/auth', { token }),
  ...
  rankProposals: (projectId, modelId) => request('POST', `/projects/${projectId}/rank-proposals`, { modelId }),
  ...
  dryRunSkill: (data) => request('POST', '/skills/dry-run', data),
  exportSkills: (ids) => request('POST', '/skills/export', { ids }),
  importSkills: (skills) => request('POST', '/skills/import', { skills }),
  getSkillLibrary: () => request('GET', '/skills/library'),
  ...
  getTaskReplayPhase: (taskId, phase) => request('GET', `/tasks/${taskId}/replay/${phase}`),
